...
const protokollSchema = z.object({
   ...
   * Signatur wurden bereits per /api/upload abgelegt). Die Action verknuepft
   * sie mit dem neuen Report - aber nur, wenn sie zu dieser Anlage gehoeren
   * und noch keinem Report zugeordnet sind.
   */
  attachmentIds: z
    .string()
    .trim()
...
type ProtokollDaten = z.output<typeof protokollSchema>;

/**
 * Schliesst ein Serviceprotokoll ab.
 *
 ...
 * die Kundenbestaetigung an.
 *
 * Spontanflow (`jobId` leer): Gehoert zur Anlage ein offener Auftrag, wird der
 * frueheste uebernommen und abgeschlossen - sonst bliebe er als Karteileiche
 * im Dashboard, obwohl die Anlage gerade gewartet wurde.
 */
export async function protokollAbschliessen(
  ...
  const daten: ProtokollDaten = parsed.data;
  ...
  // Kundenbestaetigung ist best effort: Ohne verifizierte Absenderdomain kann
  // Resend in dev nur an die eigene Account-Adresse zustellen. Ein
  // Mailfehler darf das gespeicherte Protokoll nicht kippen.
  ...
  redirect(`/anlage/${anlage.qrToken}`);
}
